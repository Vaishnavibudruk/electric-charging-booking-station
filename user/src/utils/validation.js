const isPastTime = (time) => new Date(time) < new Date();
...
export const validateBooking = ({
  ...
  if (!expectedArrivalTime) return "Please select arrival time";
  if (isPastTime(expectedArrivalTime))
    return "Arrival time cannot be in the past";

  if (!estimatedChargingDuration || estimatedChargingDuration <= 0)
    return "Enter valid charging duration";

  if (paymentMode === "ONLINE" && waitingCount > 0)
    return "Online payment not allowed when slot is full";
